...
run("Export results", "filepath=["+OUTPATH+"] fileformat=[CSV (comma separated)] firstframe=["+FIRST+"] id=false frame=true sigma=true bkgstd=true intensity=true saveprotocol=["+SAVEPROTOCOL+"] offset=true uncertainty=true y=true x=true");
close();
run("Quit");